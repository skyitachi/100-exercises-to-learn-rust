...
#[derive(Debug)]
enum TicketNewError {
    TitleErr { message: String },
    DescriptionErr { message: String },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match result {
        Ok(ticket) => ticket,
        Err(DescriptionErr { message }) => {
             Ticket::new(title_copy, "Description not provided".to_string(), status_copy).unwrap()
        }
        Err(TitleErr { message}) => panic!("{}", message)
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TitleErr { message: "Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TitleErr { message: "Title cannot be longer than 50 bytes".to_string()});
        }
        if description.is_empty() {
            return Err(DescriptionErr { message: "Description cannot be empty".to_string()});
        }
        if description.len() > 500 {
            return Err(DescriptionErr { message: "Description cannot be longer than 500 bytes".to_string()});
        }
